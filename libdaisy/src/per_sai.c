...
#define DSY_SAI_DMA_MEM_SECTION __attribute__((section(".sram1_bss")))
...
static int32_t        DSY_SAI_DMA_MEM_SECTION
                      sai1_dma_buffer_rx[DSY_SAI_DMA_BUFFER_SIZE_MAX];
static int32_t        DSY_SAI_DMA_MEM_SECTION
                      sai1_dma_buffer_tx[DSY_SAI_DMA_BUFFER_SIZE_MAX];
static int32_t        DSY_SAI_DMA_MEM_SECTION
                      sai2_dma_buffer_rx[DSY_SAI_DMA_BUFFER_SIZE_MAX];
static int32_t        DSY_SAI_DMA_MEM_SECTION
                      sai2_dma_buffer_tx[DSY_SAI_DMA_BUFFER_SIZE_MAX];
...
{
    if(sai_index == DSY_SAI_2)
    {
        return sai2_dma_buffer_rx;
    }
    else
    {
        return sai1_dma_buffer_rx;
    }
}
int32_t* dsy_sai_tx_buffer(dsy_sai_handle* hsai, uint8_t sai_index)
{
    if(sai_index == DSY_SAI_2)
    {
        return sai2_dma_buffer_tx;
    }
    else
    {
        return sai1_dma_buffer_tx;
    }
}
    ...
    if(hsai->Instance == SAI1_Block_A)
    {
        ...
        for(uint8_t i = 0; i < DSY_SAI_PIN_LAST; i++)
        {
               ...
               || sai_handle.sync_config[DSY_SAI_1] == DSY_AUDIO_SYNC_MASTER)
            {
                port = (GPIO_TypeDef*)
                    gpio_hal_port_map[sai_handle.sai1_pin_config[i].port];
                pin = gpio_hal_pin_map[sai_handle.sai1_pin_config[i].pin];
    ...
    if(hsai->Instance == SAI2_Block_A)
    {
        ...
        for(uint8_t i = 0; i < DSY_SAI_PIN_LAST; i++)
        {
               ...
               || sai_handle.sync_config[DSY_SAI_2] == DSY_AUDIO_SYNC_MASTER)
            {
                port = (GPIO_TypeDef*)
                    gpio_hal_port_map[sai_handle.sai2_pin_config[i].port];
                pin = gpio_hal_pin_map[sai_handle.sai2_pin_config[i].pin];
                HAL_GPIO_DeInit(port, pin);
            }
        }
    }
...
void HAL_SAI_RxHalfCpltCallback(SAI_HandleTypeDef* hsai)
{
    ...
    if(hsai->Instance == SAI1_Block_A)
    {
        size   = sai_handle.dma_size[DSY_SAI_1] / 2;
        sai_handle.callback[DSY_SAI_1](sai1_dma_buffer_rx + offset,
                                       sai1_dma_buffer_tx + offset,
                                       size,
                                       DSY_SAI_1);
    }
    else if(hsai->Instance == SAI2_Block_A)
    {
        size = sai_handle.dma_size[DSY_SAI_2] / 2;
        sai_handle.callback[DSY_SAI_2](sai2_dma_buffer_rx + offset,
                                       sai2_dma_buffer_tx + offset,
                                       size,
                                       DSY_SAI_2);
    }
}
...
{
#endif
    void DMA1_Stream0_IRQ_Handler(void) { HAL_DMA_IRQHandler(&hdma_sai1_a); }
    void DMA1_Stream1_IRQ_Handler(void) { HAL_DMA_IRQHandler(&hdma_sai1_b); }
    void DMA1_Stream3_IRQ_Handler(void) { HAL_DMA_IRQHandler(&hdma_sai2_a); }
    void DMA1_Stream4_IRQ_Handler(void) { HAL_DMA_IRQHandler(&hdma_sai2_b); }
#ifdef __cplusplus
} // extern C
#endif
