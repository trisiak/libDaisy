...
// Starting New CPP Version
namespace daisy
{
//  Static Buffers in SRAM1 for DMA
static int32_t __attribute__((section(".sram1_bss")))
...
void Audio::Init(dsy_sai_handle* sai,
                 dsy_i2c_handle* i2c_dev0,
                 dsy_i2c_handle* i2c_dev1)
{
    uint8_t            dev0, dev1, intext, mcu_is_master;
    I2C_HandleTypeDef *hi2c_int, *hi2c_ext;
    I2C_HandleTypeDef* hi2c_dev;
    intext      = sai_->init;
    dev0        = sai_->device[DSY_SAI_1];
    dev1        = sai_->device[DSY_SAI_2];
    hi2c_int    = dsy_i2c_hal_handle(i2c_dev0);
    hi2c_ext    = dsy_i2c_hal_handle(i2c_dev1);
    block_size_ = 6; // Temporarily hardcoded to default.
    channels_   = 2; // Fixed Stereo for now.
    dma_size_   = block_size_ * channels_ * 2; // x2 for cplt/halfcplt callbacks
    callback_   = dsy_audio_passthru;
	// Initialize Audio Peripheral
    dsy_sai_init_from_handle(sai_);
    if(sai_->init == DSY_AUDIO_INIT_SAI2 || sai_->init == DSY_AUDIO_INIT_BOTH)
    {
        dma_rx_ = sai2_dma_buffer_rx;
        dma_tx_ = sai2_dma_buffer_tx;
        device_ = dev1;
        mcu_is_master
            = sai_->sync_config[DSY_SAI_2] == DSY_AUDIO_SYNC_MASTER ? 1 : 0;
        hi2c_dev = hi2c_ext;
    }
    if(sai_->init == DSY_AUDIO_INIT_SAI1 || sai_->init == DSY_AUDIO_INIT_BOTH)
    {
        dma_rx_ = sai1_dma_buffer_rx;
        dma_tx_ = sai1_dma_buffer_tx;
        device_ = dev0;
        mcu_is_master
            = sai_->sync_config[DSY_SAI_1] == DSY_AUDIO_SYNC_MASTER ? 1 : 0;
        hi2c_dev = hi2c_int;
    }
    // Initialize Device
    // FIX so that this only initializes the I2C being used.
    dsy_i2c_init(i2c_dev0);
    dsy_i2c_init(i2c_dev1);
    switch(device_)
    {
        case DSY_AUDIO_DEVICE_WM8731:
            codec_wm8731_init(hi2c_dev, mcu_is_master, 48000.0f, 16);
            break;
        case DSY_AUDIO_DEVICE_PCM3060: break;
        default: break;
    }
    memset(dma_rx_, 0, DSY_AUDIO_DMA_BUFFER_SIZE_MAX);
    memset(dma_tx_, 0, DSY_AUDIO_DMA_BUFFER_SIZE_MAX);
    memset(in_, 0, DSY_AUDIO_BLOCK_SIZE_MAX);
    memset(out_, 0, DSY_AUDIO_BLOCK_SIZE_MAX);
    offset_ = 0;
}

void SetCallback(dsy_audio_callback cb) {}
} // namespace daisy
...
void dsy_audio_init(dsy_audio_handle* handle)
{
    ...
    audio_handle.dma_buffer_rx     = daisy::sai1_dma_buffer_rx;
    audio_handle.dma_buffer_tx     = daisy::sai1_dma_buffer_tx;
    audio_handle_ext.dma_buffer_rx = daisy::sai2_dma_buffer_rx;
    audio_handle_ext.dma_buffer_tx = daisy::sai2_dma_buffer_tx;
